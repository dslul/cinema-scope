...
void Preview::run(sc::PreviewReplyProxy const &reply) {
    ...
    QJsonDocument root;
    int j = 0;
    std::string ytsource = "", movie_or_tv = result["movie_or_tv"].get_string(),
                append, runtime, genres, networkstr, caststr, director,
                in_production, usr1str, usr2str, tagline, overview, imdburi,
    ...
    if (isMovie && !isActor) {
        ...
        trail2 = "youtube";
        trail3 = "source";
        append = "trailers,reviews,credits";
        networks = "production_companies";
        networkstr = "<b>Production companies:</b> ";
    } else if (!isMovie && !isActor) {
        trail1 = "videos", trail2 = "results", trail3 = "key";
        append = "videos,credits";
        networks = "networks";
        networkstr = "<b>Networks:</b> ";
    } else
        append = "movie_credits";
    client_.get({result["movie_or_tv"].get_string(), result["id"].get_string()},
                {{"api_key", client_.config_->moviedb_key},
                 {"append_to_response", append},
                 {"language", result["lang"].get_string()}},
                root, client_.config_->moviedbroot);
    /**
    ...
    if (!(trailers.toList().isEmpty())) {
        ...
        ytsource = "https://www.youtube.com/watch?v=" +
                   traileritem[trail3].toString().toStdString();
    }
    if (!isActor) {
        ...
        if (isMovie) { // movie info
            ...
            release_date =
                filmdate.toString(Qt::SystemLocaleLongDate).toStdString();
            ...
            if (usr1str.empty() || usr1str == "0")
                ...
                usr1str = "<b>Budget:</b> " + usr1str;
            usr2str = infoitem["revenue"].toString().toStdString();
            if (usr2str.empty() || usr2str == "0")
                usr2str = "<b>Revenue:</b> unknown";
            else
                usr2str = "<b>Revenue:</b> " + usr2str;
            imdburi = "http://www.imdb.com/title/" +
                      infoitem["imdb_id"].toString().toStdString();
        } else { // tv show info
            ...
            airdate =
                seriesdate.toString(Qt::SystemLocaleLongDate).toStdString();

            in_production =
                (infoitem["in_production"].toString().toStdString() == "true")
                    ? "✔"
                    : "no";
            // other
    ...
    if (!isActor)
        layout1col.add_column({"headerId", "videoId", "ratingId", "genresId",
                               "castId", "directorId", "reldateId", "expId",
                               "statusId", "runtimeId", "networksId", "usr1Id",
                               "usr2Id", "summaryId", "actionsId", "revtitleId",
                               "reviewsId"});
    else
        layout1col.add_column(
            {"headerId", "imageId", "galleryId", "summaryId", "actionsId"});

    // Two column layout
    if (!isActor) {
        layout2col.add_column({"videoId", "ratingId", "genresId", "castId",
                               "directorId", "reldateId", "expId", "statusId",
                               "runtimeId", "networksId", "usr1Id", "usr2Id"});
        layout2col.add_column(
            {"headerId", "summaryId", "actionsId", "revtitleId", "reviewsId"});
    } else {
        layout2col.add_column({"imageId", "galleryId"});
    ...
    if (tagline.empty())
        ...
        w_summary.add_attribute_value("title",
                                      sc::Variant("<i>" + tagline + "</i>"));
    ...
    if (isActor == false) {
        ...
        w_genres.add_attribute_value("text",
                                     sc::Variant("<b>Genres:</b> " + genres));
        w_cast.add_attribute_value("text",
                                   sc::Variant("<b>Cast:</b> " + caststr));
        w_director.add_attribute_value(
            "text", sc::Variant("<b>Director:</b> " + director));
        rat_builder.add_tuple(
            {{"rating",
              sc::Variant(floor(infoitem["vote_average"].toFloat() + 0.5) /
                          2)}});
        w_rat.add_attribute_value("reviews", rat_builder.end());
        w_status.add_attribute_value(
            "text", sc::Variant("<b>Status:</b> " +
        ...
        if (isMovie) {
            w_reldate.add_attribute_value(
                "text", sc::Variant("<b>Release date:</b> " + release_date));
            w_runtime.add_attribute_value(
                "text", sc::Variant("<b>Runtime:</b> " + runtime));
            w_revtitle.add_attribute_value("title",
                                           sc::Variant("<b>Reviews</b>"));
            QVariantMap item, inforeviews = infoitem["reviews"].toMap();
            for (const QVariant &i : inforeviews["results"].toList()) {
                item = i.toMap();
                rev_builder.add_tuple(
                    {{"author",
                      sc::Variant("<b>" +
                                  item["author"].toString().toStdString() +
                                  "</b>")},
                     {"review",
                      sc::Variant(item["content"].toString().toStdString())}});
            }
            if (!item.isEmpty())
                w_reviews.add_attribute_value("reviews", rev_builder.end());
            else
                w_revtitle.add_attribute_value(
                    "text", sc::Variant(_("No reviews avaiable.")));
            // TV SHOWS specific sections
        } else {
            w_reldate.add_attribute_value(
                "text", sc::Variant("<b>First air date</b>: " + airdate));
            w_runtime.add_attribute_value(
                "text", sc::Variant("<b>In production</b>: " + in_production));
        }
        ...
        for (const QVariant &i : inforeviews["cast"].toList()) {
            ...
            if (!tmp.empty())
                imgarr.push_back(
                    sc::Variant("http://image.tmdb.org/t/p/w154" + tmp));
        }
    ...
    if (isMovie && (QDate::currentDate() > filmdate &&
                    QDate::currentDate().year() - filmdate.year() <= 1))
        act_builder.add_tuple({{"id", sc::Variant("oncinema")},
                               {"label", sc::Variant(_("Find cinema"))},
                               {"uri", result["uri"]}});
    if (homestr != "")
        act_builder.add_tuple({{"id", sc::Variant("homepg")},
    ...
    if (imdburi != "" && imdburi != "http://www.imdb.com/title/" &&
        imdburi != "http://www.imdb.com/name/")
        act_builder.add_tuple({{"id", sc::Variant("imdb")},
    ...
    if (isActor) {
        reply->push({w_header, w_image, w_gallery, w_summary, w_actions});
    } else {
        reply->push({w_header, w_video, w_rat, w_expandable, w_summary,
                     w_actions, w_revtitle, w_reviews});
    }
}
